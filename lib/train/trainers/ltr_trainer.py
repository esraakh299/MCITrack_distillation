...
class LTRTrainer_adapt(BaseTrainer):
    """Trainer for TAD distillation with separate optimizers for student and adaptive net."""

    def __init__(self, actor, loaders, optimizer, optimizer_adapt, settings,
                 lr_scheduler=None, lr_scheduler_adapt=None, use_amp=False):
        super().__init__(actor, loaders, optimizer, settings, lr_scheduler)
        self._set_default_settings()
    ...
    def save_checkpoint(self):
        ...
        adaptive_net = self.actor.adaptive_net.module if multigpu.is_multi_gpu(
            self.actor.adaptive_net) else self.actor.adaptive_net
    ...
    def load_checkpoint(self, checkpoint=None, fields=None, ignore_fields=None, load_constructor=False):
        ...
        ignore_fields.extend(['lr_scheduler', 'lr_scheduler_adapt', 'constructor',
                              'net_type', 'actor_type', 'net_info'])
    ...
    def cycle_dataset(self, loader):
        """Do a cycle of training or validation with two-pass backward."""
        self.actor.train(loader.training)
        torch.set_grad_enabled(loader.training)
        ...
        for i, data in enumerate(loader, 1):
            ...
            # forward pass (returns loss, loss_adapt, stats)
            if not self.use_amp:
